...
var streamersTracker = {
    ...
    T1 : {channelId: "51496027", emoji: '', status: 'offline', URL: ":tyler1Thinking:", viewers: 0, MoreThan10kPostedDiscord: false}
};
...
function postSummary(channel){
    ...
    for (data in streamersTracker) {
        // console.log(streamersTracker[data])
        if (streamersTracker[data].status == "Online") {
            embed.addField(data + " - " + streamersTracker[data].emoji, "[link -- replace with title here]("+ streamersTracker[data].URL + ")")
        }
    }
